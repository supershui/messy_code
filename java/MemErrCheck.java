...
public class MemErrCheck {
	...
	private static class ErrRec {
		...
		public ErrRec(int src, int dest, int mode, boolean random) {
			this.src = src;
			this.dest = dest;
			this.mode = mode;
			this.random = random;
		...
		public String toString() {
			return "ErrRec{src:" + intToBinStr(src) + ";dest:" + intToBinStr(dest) + ";mode:" + mode + ";random:" + random + "}";
		}
	}
	...
	public void check() {
		...
		for (int i = 0; i < mRounds; i++) {
			...
			logln("check: round " + i + " result " + roundResult + "; time cost " + ((loopEnd-loopBegin)/1000) + " us.");

			overallResult = overallResult && roundResult;

		}
		long end = System.nanoTime();
		logln("check: overall result " + overallResult + "; time cost " + ((end-begin)/1000) + " us.");
	...
	private boolean checkMemWithMode(int[][] mem, int mode) {
		for (int i = 0; i < mem.length; i++) {
			...
			for (int j = 0; j < len; j++) {
				if (mem[i][j] != mode) {
					logln("checkMemWithMode: mem[" + i + "][" + j + "] failed, expected " + intToBinStr(mode) + "; got " + intToBinStr(mem[i][j]));
					return false;
				}
			}
		}
	...
	private void saveToErrorRecords(int src, int dest) {
		ErrRec er = new ErrRec(src, dest, mCurrentMode, mCurrentRandom);
		...
		for (int i = 0; i < source.length; i++) {
			...
			for (int j = 0; j < len; j++) {
				if (source[i][j] != destination[i][j]) {
					logln("checkArrays: src[" + i + "][" + j + "]=" + intToBinStr(source[i][j]) + "; dest[" + i + "][" + j + "]=" + intToBinStr(destination[i][j]));
	...
	private boolean checkWithRandom() {
		...
		log("checkWithMode: checking destionation for " + rounds + " loops ......");
